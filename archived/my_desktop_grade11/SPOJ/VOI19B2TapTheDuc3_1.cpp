...
void editDanhDau(int x, int y){
	...
	while (!q.empty()){
		...
		for (auto& it: unEdge[y][nodeU]){
			...
			long long int distUV = it.second.first.first;
			if (danhdaucanhK[wayU][y]&&danhdaucanhK[wayV][y]) prefixWay[wayV][y] = prefixWay[wayU][y] + distUV;
	...
	for (int i = 0; i < topo.size(); i++) for (auto it : unEdge[1][topo[i]]){
		...
		for (int j = 0; j < (1<<(soBanDeTinh)); j++){
			...
			for (int bitmask = 0; bitmask < (1<<(soBanDeTinh)); bitmask++){
				...
				bool coAiDeDiQuaKhoKhong = false;
				int x = j,y = bitmask;
				bool check = true;
				for (int t = 1; t <= soBanDeTinh; t++){
					...
					if (coAiKhoTinhDiQuaHong&&danhdaucanhK[wayV][thuTuBanDeTinh[t]]) coAiDeDiQuaKhoKhong = true;
					if (y%2>x%2) check = false; 
					x /= 2;
					y /= 2;
				}
				//if (coAiKhoTinhDiQuaHong&&!coAiDeDiQuaKhoKhong) distUV = max(distUV,it.second.first.first);
